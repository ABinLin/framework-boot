package com.farerboy.framework.boot.redis.manager;
...
import com.farerboy.framework.boot.redis.constant.RedisConstant;
...
public class RedisManager {
    ...
    public <T> T get(String key, Class<T> clazz){
        ...
        try {
            ...
            throw new BaseException(RedisConstant.ERROR_CODE,"对象("+o+")转JsonString失败",e);
        }
        try {
            T t=JSONObject.parseObject(jsonStr,clazz);
            return t;
        }catch (Exception e){
            throw new BaseException(RedisConstant.ERROR_CODE,"json字符串("+jsonStr+")转"+clazz+"对象失败",e);
        }
    }
    ...
    public Long increment(String key, long delta) {
        if (delta < 0) {
            throw new ValidException("递增因子必须大于0");
        }
     ...
     */
    public boolean hashSetMany(String key, Map<String, Object> map) {
     ...
     * @return true成功 false失败
     */
    public boolean hashSetMany(String key, Map<String, Object> map, long time) {
        redisTemplate.opsForHash().putAll(key, map);
        if (time > 0) {
            expire(key, time);
        }
